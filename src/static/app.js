  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // build participants HTML if any exist
        let participantsHTML = "";
        if (details.participants && details.participants.length) {
          participantsHTML = `<p><strong>Participants:</strong></p>
            <ul class=\"participants-list\">` +
            details.participants.map(p => `
              <li class=\"participant-item\">${p} <span class=\"remove-participant\" data-activity=\"${name}\" data-email=\"${p}\">🗑️</span></li>`
            ).join("") +
            `</ul>`;
        }
        ...
        // wire up delete buttons on this card
        activityCard.querySelectorAll('.remove-participant').forEach(icon => {
